...
double MappabilityClass::GetMappability(string chr, int start, int end){ //rlength = region length
    ...
    if(abs(end - start) > 200)
        NumberofBits = abs((end -start)) / span; // How many bits to read
    else
        NumberofBits = 1;
...
if(file.is_open())
{
	...
	FindBinaryPos(chroffset, chroffset_next, start, offset, bitcount);
